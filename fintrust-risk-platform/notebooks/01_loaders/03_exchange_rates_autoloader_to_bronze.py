...
from src.common.contracts import EXCHANGE_RATES_CONTRACT
...
landing_path     = volume_path(config, "landing_base_path", "exchange_rates")
checkpoint       = checkpoint_path(config, config["sources"]["exchange_rates_file"]["checkpoint_name"])
target_table     = table_name(config, "bronze", "exchange_rates_raw")
quarantine_table = table_name(config, "quarantine", "invalid_exchange_rates")
...
required_columns = ["currency", "rate_date", "rate_to_cad"]
...
try:
    ...
    write_reconciliation_result(spark, recon_table, "exchange_rates_autoloader", "landing_to_bronze", source_count, target_count)
    write_pipeline_health(spark, health_table, "exchange_rates_autoloader", "bronze", run_id, "SUCCESS", start_time, datetime.now(timezone.utc), records_processed, records_quarantined)
except Exception as e:
    write_pipeline_health(spark, health_table, "exchange_rates_autoloader", "bronze", run_id, "FAILED", start_time, datetime.now(timezone.utc), error_message=str(e))
    raise
